...
void testApp::setup() {
    ...
    ofSetFrameRate(150);
	click=0;		// to detect if mouse clicked
...
void testApp::draw(){
    ...
    //draw red circles for found blobs
		...
		for (int i=0; i<contours[j].nBlobs; i++) 
		{
			...
			//ofLine(px[i],py[i],contours[j].blobs[i].centroid.x*3.2, contours[j].blobs[i].centroid.y*3.2);
			//cout<<"\n"<<contours[j].blobs[i].centroid.x;
			//cout<<"\t"<<contours[j].blobs[i].centroid.y;
			//px[i]=contours[j].blobs[i].centroid.x * 3.2;
			//py[i]=contours[j].blobs[i].centroid.y * 3.2;
			ofSetColor(0,0,0,20);
			ofRect(0,0,ofGetWidth(),ofGetHeight());
			ofSetColor(255,255,255,50);
			ofCircle(contours[j].blobs[i].centroid.x*3.2, contours[j].blobs[i].centroid.y*3.2, 20);
