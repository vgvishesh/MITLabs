...
#include <math.h>
#include <stdlib.h>
#define BEZIER_CONTROL_POINTS 3
...
float eucliedian(float ax,float ay, float bx, float by)
{
	float dist= sqrt(pow(ax-bx,2) + pow(ay-by,2));
	return dist;
}

void distanceLimiter(float ax,float ay, float &bx, float &by)
{
	float dx=ax-bx;
	float dy=ay-by;
	if(abs(dx)<=WIDTH/4)
		return;
	if(abs(dx)<=WIDTH/2 && abs(dx)>WIDTH/4)
		dx=dx/2;
	else if(abs(dx)<=3*WIDTH/4 && abs(dx)>WIDTH/2)
		dx=dx/3;
	else if(abs(dx)<=WIDTH && abs(dx)>3*WIDTH/4)
		dx=dx/4;
	...
	if(abs(dy)<=HEIGHT/4)
		return;
	if(abs(dy)<=HEIGHT/2 && abs(dy)>HEIGHT/4)
		dy=dy/2;
	else if(abs(dy)<=3*HEIGHT/4 && abs(dy)>HEIGHT/2)
		dy=dy/3;
	else if(abs(dy)<=HEIGHT && abs(dy)>3*HEIGHT/4)
		dy=dy/4;
...
void testApp::draw(){
    ...
    //draw red circles for found blobs
		...
		{
			ofSetColor(color[k],50);
			//ofSetColor(rand()%255,rand()%255,rand()%255,50);
			pos=start[k];
			int j=0;
			while(pos!=NULL && pos->x!=-4 && pos->y!=-4 && (elementCount[k]-j*BEZIER_CONTROL_POINTS - 1)>BEZIER_CONTROL_POINTS)
			{
				int z=0;
				while(z<BEZIER_CONTROL_POINTS -1 )
				...
				bezier(15,BEZIER_CONTROL_POINTS);

				for(int i=1;i<=15;i++)
					ofLine(bz[i-1].x,bz[i-1].y,bz[i].x,bz[i].y);

				// FOR INVERTED MIRROR IMAGE ALONG X , Y AXIS...............
				/*for(int i=0;i<=15;i++)
					ofLine(WIDTH-bz[i-1].x,HEIGHT-bz[i-1].y,WIDTH-bz[i].x,HEIGHT-bz[i].y);*/
				j++;
			}
			//pos=start[k];
			//while(pos!=NULL)
